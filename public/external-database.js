    ...
    try {
        ...
        if (localData) {
            ...
            if (Array.isArray(data) && data.length > 0) {
                // Validação extra: verificar se tem preço (evita crash com dados antigos do Firebase)
                if (data[0].price !== undefined || data[0].preco !== undefined) {
                    console.log('📦 Usando backup do localStorage:', data.length, 'produtos');
                    __camisasCache = data;
                    __camisasCacheTs = Date.now();
                    return data;
                } else {
                    console.warn('⚠️ Backup do localStorage inválido (sem preço), ignorando.');
                    localStorage.removeItem('camisas_backup');
                }
            }
        }
    } catch (error) {
